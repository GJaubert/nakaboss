...
impl Client {
    ...
    pub fn handle(&self) -> TestHandle {
        TestHandle {
            tip: (0, self.network.genesis(), Uint256::from_be_bytes(self.network.genesis().work().to_be_bytes())),
    ...
    pub fn received(&mut self, remote: &net::SocketAddr, payload: NetworkMessage) {
        let msg = RawNetworkMessage::new(Magic::from_bytes(self.network.magic().to_be_bytes()), payload);
