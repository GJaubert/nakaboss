        ...
        let next = self
            ...
            .map(|io| match io {
                output::Io::Write(addr, payload) => Io::Write(
                    addr,
                    RawNetworkMessage::new(Magic::from_bytes(
                        self.network.magic().to_be_bytes()), payload),
                ),
    ...
    for StateMachine<T, F, P, C>
{
    ...
    fn message_received(&mut self, addr: &net::SocketAddr, msg: Cow<RawNetworkMessage>) {
        ...
        if *msg.magic() != Magic::from_bytes(self.network.magic().to_be_bytes()) {
            return self
                .peermgr
                .disconnect(addr, DisconnectReason::PeerMagic(
                    u32::from_be_bytes(msg.magic().to_bytes())));
        }
